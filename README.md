...
### 4.1. Troubleshooting / tracing

It can be useful to add some logging if errors are seen in the app; note that the container image needs to be rebuilt when `app.py` is changed so that it runs the updated code.

#### 4.1.1. Turn on DEBUG for Microsoft OpenTelemetry
...
#### 4.1.2. Verifying query sent to hunting tool

Add this line before `response = ...` under `_run_hunting_query` in `app/app.py`:

```python
        logging.info("Running hunting query with timespan=%s workspace_id=%s:\n%s", timespan, workspace_id, query)
```

This logs the query sent to run hunting query API.
